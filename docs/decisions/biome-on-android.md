...
| `packageExtensions`, as was done for the yuku bindings (retired once upstream published them) | The gate reads the target package's own manifest, so grafting the official tarball changes nothing. It would need a repacked tarball declaring `os: [android]`, hosted and re-cut on every Biome bump. |
